<template>
  <div>
    ...
    <el-form-item label="价格限制方式">
      <x-radio v-model="viewModel.limit.priceLimitType" type="PriceLimitType"></x-radio>
    ...
    <el-form-item label="最小触发金额">
      <el-input-number v-model="viewModel.limit.minimumAmount" :precision="2" :min="0" :step="1"></el-input-number>
      <div class="demo_intro">分润的最小触发金额，当用户下单金额小于最小触发金额时不会触发分润</div>
    </el-form-item>
    <el-form-item label="最大触发金额" :required="true">
      <el-input-number v-model="viewModel.limit.maxAmount" :precision="2" :min="0" :step="1"></el-input-number>
      <div class="demo_intro">分润的最大触发金额，当用户下单金额大于最小触发金额时不会触发分润（等于时触发),0表示不限制</div>
    </el-form-item>
    <el-form-item label="分润触发类型" :required="true">
      <x-radio v-model="viewModel.limit.triggerType" type="TriggerType"></x-radio>
      <div class="demo_intro">用户或用户类型升级：用户或用户类型升级:比如会员注册、会员升级，省代理加盟、门店开通等。商城购物订单：指商城系统下单</div>
    </el-form-item>
    <div v-if="viewModel.limit.triggerType === 2">
      <x-line :border="true" title="商品范围" type="metal" icon="flaticon-rocket" style="padding:0px 15px;">
      </x-line>
      <el-form-item label="分润价格类型">
        <x-radio v-model="viewModel.goodRang.amountType" type="PriceType"></x-radio>
      ...
      <el-form-item label="商品依赖范围">
        <x-radio v-model="viewModel.goodRang.productModel" type="ProductModelType"></x-radio>
        <div class="demo_intro">选择商品的限制范围</div>
      </el-form-item>
      <el-form-item label="所属产品线" v-if="viewModel.goodRang.productModel==1">
        <x-checkbox v-model="viewModel.goodRang.productLines" :widget="checkValue">
        </x-checkbox>
      ...
      <el-form-item label="所属商城" v-if="viewModel.goodRang.productModel==2">
        <x-radio v-model="viewModel.goodRang.priceStyleId" type="PriceStyleConfig"></x-radio>
