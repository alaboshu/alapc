        ...
        <div class="order_list" v-for="(item,itemIndex) in user.data.result.result" :key="itemIndex">
          ...
          <div class="order_list_cont" v-for="(cont,contIndex) in item.outOrderProducts" :key="contIndex">
            ...
            <div class="order_list_cont_right" v-if="contIndex===0">
              ...
              <div class="order">
                <p class="button">{{item.orderStatuName}}
                </p>
                <p>
                  <router-link class="iconfont zk-userfeedback" tag="i" :to="goOrderShow(user.data.tableActions[0].url,item.id)">订单详情</router-link>
                </p>
  ...
  export default {
    ...
    methods: {
      async  init () {
        ...
        if (this.widgetData.value !== null && this.widgetData.value.title !== undefined) {
          this.borderTitle = this.widgetData.value.title
        }
        this.async = true
        setTimeout(() => {
      ...
      goOrderShow (url, id) {
        // if (this.$route.matched[0].path.indexOf('/admin') !== -1) {
        //   if (url.indexOf('/user') !== -1) {
        //     url = url.replace('/user', this.$route.matched[0].path)
        //     return url
        //   }
        //   if (url.indexOf('/User') !== -1) {
        //     url = url.replace('/User', this.$route.matched[0].path)
