...
<script>
  ...
  // import local from '@/service/pc/local.js'
    ...
    methods: {
      ...
      async getDialog (method) {
        ...
        if (method === 'UserOrderCheckLogistics') {
          ...
          var checkLogistics = await action.userOrderCheckLogistics(this, par)

          this.LogisticsMessage = checkLogistics.result.list
          this.checkLogistics = true
        } else {
          var this_ = this
          this.TableEdit = true
          this.$nextTick(function () {
            this_.$refs.from_item.changeover(method)
          })
        }
      },
      async cancelOrder () {
      ...
      async saveTableData () {
        ...
        if (this.$refs.from_item.currt === 'OrderAdminDelivery') {
          paramenter.ExpressNum = user.logistics
          paramenter.ExpressNo = user.ExpressNo
          paramenter.ExpressName = user.ExpressName
          paramenter.OrderId = this.viewModel.id
          paramenter.StoreId = this.viewModel.storeId
          paramenter.UserId = this.$user.id()
          paramenter.Name = user.name
          paramenter.Address = user.address
          paramenter.RegionId = user.regionId
          paramenter.UserName = this.viewModel.order.orderExtension.user.userName
        }
        if (this.$refs.from_item.currt === 'OrderAdminChangeAddress') {
          paramenter.RegionId = user.regionId
          paramenter.Address = user.address
        ...
        if (this.$refs.from_item.currt === 'OrderAdminRemark') {
          paramenter.OrderId = this.viewModel.id
        }
        if (this.$refs.from_item.currt === 'OrderAdminPay') {
        ...
        if (this.$refs.from_item.currt === 'Api/Refund/Process') {
          var refundData = {
            productId: this.viewModel.productSkuItems[0].productId,
            storeId: this.viewModel.storeId,
            userId: this.viewModel.userId,
            orderId: this.viewModel.order.id
          }
          var refundRes = await this.$api.httpPost('api/refund/process', refundData)
          this.$crud.message(this, refundRes)
        }
        if (this.$refs.from_item.currt === 'AdminAllowRefund') {
          dataGet.address = this.$refs.from_item.returnAddress
          var AllowProcess = await this.$api.httpPost('api/refund/AllowProcess', dataGet)
          this.$crud.message(this, AllowProcess)
        }
        if (this.$refs.from_item.currt === 'AdminRefuseRefund') {
          dataGet.processMessage = this.$refs.from_item.processMessage
          var RefuseProcess = await this.$api.httpPost('api/refund/RefuseProcess', dataGet)
          this.$crud.message(this, RefuseProcess)
        }
        action.action(this, paramenter, this.$refs.from_item.currt)
      },
      saveCancel () {
        var user = this.$refs.from_item.user
